...
ParallelForTask::ParallelForTask(char* inFN, char* outFN)
	: inFileName(inFN), outFileName(outFN)
{
	openFile();
	readFile();
	inFile.close();
...
ParallelForTask::~ParallelForTask(){}

void ParallelForTask::openFile()
{
...
void ParallelForTask::writeInFile()
{
...
void ParallelForTask::doTask()
{
	...
	A.parallelForMultiplication(B, resultMatrix);
